function SelCity(obj,e) {
    ...
    var dal = '<div class="_citys">' +
        ...
        '<li class="citySel">省份</li><li>城市</li>' +
        '</ul>' +
    ...
    var tb_province = [];
    var h=[];
    var th_province = ['全部','澳门','台湾省','香港','北京市','天津市','上海市','重庆市'];
    var b = province;

    // 获取自定义数组内容，添加到内容块中
    for(let n=0;n<th_province.length;n++){
        h.push('<a class="null">'+ th_province[n] +'</a>')
    }
    $("#_citys0").append(h.join(""));

    for (var i = 0, len = b.length; i < len; i++) {
        tb_province.push('<a data-level="0" data-id="' + b[i]['id'] + '" data-name="' + b[i]['name'] + '">' + b[i]['name'] + '</a>');
    }
    $("#_citys0 a:first-child").addClass("AreaS");
    $("#_citys0 .null").click(function () {
        ths.value = $(this).text();
        Iput.colse();
    }); // 给自定义数组添加事件
